...
**프로젝트명**: NOAH (노아) — **N**etworked **O**perations **A**gent **H**ub
**작성자**: agent_crew
**작성일**: 2026-04-13
**상태**: Phase 1 설계 확정, 구현 대기 (NeoStock Phase 7 이후 시작)
**목표**: 사람 + AI 에이전트가 함께 사용하는 P2P 메신저
...
**NOAH** — 두 가지 의미를 동시에 가집니다:

#### 엔지니어링 의미
**N**etworked **O**perations **A**gent **H**ub  
...
**NOAH는 사람과 AI 에이전트가 서로 메시지를 빠르게 전달**합니다.
...
**핵심**: NOAH는 "AI 전용"이 아니라 **"사람도 쓸 수 있는 메신저인데, AI 통합이 일급 시민"**.
...
- **태그라인**: 
  - 영문: "Networked Operations Agent Hub"
  - 한글: "사람과 AI를 위한 메신저"
  - 부제: "방주에서 보낸 메시지" / "Messages from the Ark"
...
각 AI는 새의 종류로 비유. 모두 노아의 방주에 모인 새들.

### 0.5 네이밍 규칙

```
프로젝트명: NOAH (대문자 권장, "Noah"도 허용)
폴더명:    Noah
GitHub:    godinus123/Noah
EXE:       Noah_{DeviceName}.exe
패키지:    com.noah.anmok (Android)
서버 dir:  /home/godinus/Noah/
DB:        noah_server.db
```
...
## 10.6 PDF 자체 뷰어 (PDFium 기반)

### 배경

AI 에이전트가 분석 보고서, 코드 문서, 리포트를 PDF로 자주 보냅니다.  
매번 외부 앱(Acrobat 등)을 띄우면 UX가 끊기므로 **Noah 안에서 직접 표시**.

### 라이브러리 선택: PDFium

```
이름:      PDFium
출처:      Google (Chrome PDF 엔진)
라이선스:  BSD-3 Clause (MIT 호환)
크기:      약 8 MB (native binary)
지원:      Windows, macOS, Linux, Android, iOS
래퍼:      PdfiumViewer (.NET, Apache 2.0)
```

선택 이유:
- ✅ Chrome이 사용 → 안정성/성능 검증됨
- ✅ MIT 호환 라이선스
- ✅ 텍스트 추출 가능 (검색/복사 지원)
- ✅ 작은 크기 (8 MB)
- ✅ 멀티플랫폼
...
| 메타데이터 | ✅ | 제목/저자/생성일 |
| 폼 채우기 | ⚠️ | 가능하나 별도 작업 |
| 주석 (하이라이트/메모) | ❌ | Phase 3에서 |
| 텍스트 편집 | ❌ | PDFium은 read-only |
| 디지털 서명 | ❌ | |
| OCR (스캔 PDF 텍스트화) | ❌ | Tesseract 별도 |

**Phase 1B 범위**: 읽기 + 텍스트 복사/검색 (가장 자주 쓰는 기능).
...
│📄│   └────────────────────┘        │
├──┴──────────────────────────────────┤
│  ◀  5 / 12  ▶   100%  🔍 [검색...]│  ← 푸터 (페이지/줌/검색)
└─────────────────────────────────────┘

좌측: 페이지 썸네일 사이드바 (toggle)
중앙: 현재 페이지 (확대/축소/스크롤)
우상단: 검색, 설정, 저장, 닫기
하단: 페이지 네비, 줌, 검색바
```
...
```
1. 사용자가 PDF 안 텍스트 드래그
2. 선택된 텍스트 하이라이트 (파란색)
3. 컨텍스트 메뉴 표시:
   [복사] [검색] [번역] [공유]
4. "복사" 클릭 → 클립보드
5. "공유" 클릭 → "이 텍스트로 메시지 보내기" (선택한 채팅방으로)
```

### 메모리 관리

큰 PDF (100MB+, 1000페이지)는 메모리 부담:

```
전략 1: 페이지 lazy load
  - 현재 페이지 + 앞뒤 2페이지만 메모리에
  - 나머지는 디스크에 저장
  
전략 2: 썸네일 캐시
  - 모든 페이지의 작은 썸네일 (50x70) 미리 생성
  - 큰 캐시는 LRU로 관리
...
| PDF 크기 | 페이지 수 | 첫 페이지 표시 시간 | 메모리 |
|---------|---------|--------------------|---------| 
| 100 KB | 1~5 | 100ms | 30 MB |
| 1 MB | 10~30 | 300ms | 80 MB |
| 10 MB | 50~100 | 1초 | 200 MB |
| 50 MB | 200~500 | 3초 | 500 MB |
| 100 MB | 1000+ | 5초 | 1 GB (lazy load 필요) |

### 플랫폼별 통합

#### 모바일 (MAUI Android)
```csharp
// PdfiumViewer.Maui (또는 직접 binding)
<pdfium:PdfView Source="{Binding PdfFilePath}"
...
라이브러리:
- `PdfiumViewer.Maui` (NuGet)
- 또는 native PDFium binding

#### PC (WPF)
```csharp
// PdfiumViewer (WinForms 기반, WPF에 wrap)
<WindowsFormsHost>
  <pdfium:PdfViewer x:Name="pdfViewer" />
</WindowsFormsHost>
```

라이브러리:
- `PdfiumViewer` (NuGet, .NET WinForms)
- WPF는 `WindowsFormsHost`로 래핑

#### 서버 (Node.js)
```javascript
// 서버는 PDF 메타데이터 추출 + 첫 페이지 썸네일만 생성
// (사용자가 명시적으로 요청 시)
const PDFParser = require('pdf-parse');
const data = await PDFParser(fs.readFileSync('file.pdf'));
console.log(data.numpages, data.text.substring(0, 200));
```

서버는 PDF 본문은 거의 다루지 않음 (배달 후 삭제 정책).

### 보안 고려사항

PDF도 위험이 있습니다 (XSS와 비슷):

```
위험:
- PDF 안 JavaScript (PDFium은 기본 비활성화 ✅)
- 외부 링크 (클릭 시 확인)
- 폼 제출 (외부 도메인으로 데이터 전송 위험)
- 임베디드 파일 (악성 코드)

방어:
- PDFium 옵션: javascript: false, allow_external_links: false
- 외부 링크 클릭 시 확인 다이얼로그
- 폼 제출 차단
- 임베디드 파일 자동 추출 안 함
```
...
```json
{
  ...
  "metadata": {
    ...
    "thumbnail_path": "thumbnails/abc123_thumb.png"
  },
  "text_preview": "이 보고서는...(첫 200자)"
}
```

서버는 메타데이터 + 썸네일만 큐에 저장. 본문은 첨부 파일로 별도.

### 부가 기능 (Phase 1B)

- **PDF 검색**: 채팅방 검색 시 PDF 본문도 검색 가능
- **PDF → 텍스트**: AI에게 "이 PDF 분석해줘" 요청
- **PDF → 이미지**: 특정 페이지를 이미지로 변환해서 메시지로 보내기
- **메타데이터 자동 표시**: 제목/저자가 메시지에 자동 표시
...
수용 가능한 수준 (카카오톡 250MB와 비교 시).

### 작업 시간 (Phase 1B 추가)

| 작업 | 시간 |
|------|------|
| PdfiumViewer 패키지 통합 | 1h |
| 인라인 메시지 미리보기 (썸네일) | 2h |
| 모달 뷰어 UI (모바일) | 3h |
| 모달 뷰어 UI (PC) | 3h |
| 텍스트 선택/복사 | 1h |
...
Phase 1B에 추가하면 총 28h.

### 향후 확장 (Phase 2+)

- **주석 도구** (하이라이트, 노트)
- **OCR** (스캔 PDF 텍스트화, Tesseract)
- **PDF 편집** (페이지 추가/삭제/회전, PDFsharp 추가)
- **DOCX/XLSX/PPTX 뷰어** (Office 문서 통합)
